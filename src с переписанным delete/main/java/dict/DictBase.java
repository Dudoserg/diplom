...
//    }
     ...
     *
     * @param del вершину которую необходимо удалить
     */
    int cc = 0;

    public void deleteVertex(Vertex del) throws DictException {
        cc++;

        //DEL -> vertex1,vertex2,vertex3
        EdgeMap forDelInInvert = map.get(del);

        // в инвертированном списке удаляем инфу вида  vertex1 -> del ||| vertex2->del
        if (forDelInInvert != null)
            for (Map.Entry<Vertex, Edge> el : forDelInInvert.getEdgeMap().entrySet()) {
                Vertex vertex1 = el.getKey();
                invertMap.get(vertex1).getEdgeMap().remove(del);
            }

        // в обучном списке удаляем инфу вида del->vertex1,vertex2,vertex3
        map.remove(del);


        /// в инвертированном списке удаляем инфу вида del-> A, А1, А2
        EdgeMap edgeMap1 = invertMap.get(del);
        invertMap.remove(del);

        if (cc == 2)
            System.out.println();
        /// в обычном списке удаляем инфу вида А1 -> del, A2 -> del
        for (Map.Entry<Vertex, Edge> el : edgeMap1.getEdgeMap().entrySet()) {
            Vertex A1 = el.getKey();
            try {
                if (map.get(A1) != null)
                    map.get(A1).getEdgeMap().remove(del);
...
//        EdgeMap invertEdgeMap = invertMap.get(del);
...
//                mapWhereNeedToRemove.getEdgeMap().remove(del);
//            }
//            invertMap.remove(del);
//        }

//        EdgeMap removedInvertMap = this.invertMap.remove(del);
