...
//        );
       ...
       // Vertex zavedenie = dictBase.getVertex("заведение");
        //dictBase.deleteVertex(zavedenie);
...
//                );
    ...
    public static void wtf(DictBase dictBase){
        for (Map.Entry<Vertex, EdgeMap> vertexEdgeMapEntry : dictBase.getMap().entrySet()) {
            Vertex key = vertexEdgeMapEntry.getKey();
            EdgeMap value = vertexEdgeMapEntry.getValue();
            if("заведение".equals(key.getWord().getStr()))
                System.out.println();
            for (Map.Entry<Vertex, Edge> vertexEdgeEntry : value.getEdgeMap().entrySet()) {
                Vertex key2 = vertexEdgeEntry.getKey();
                Edge edge = vertexEdgeEntry.getValue();

                if("заведение".equals(key2.getWord().getStr()))
                    System.out.println();

                if("заведение".equals(edge.getFrom().getWord().getStr()))
                    System.out.println();
                if("заведение".equals(edge.getTo().getWord().getStr()))
                    System.out.println();
            }
        }
